...
int cbor_decode_att_obj(const u8 *cbor_data, size_t cbor_data_len,
                        struct fido_data *data) {
    // The attestation object is a CBOR map, with the following keys:
    // - fmt: a text string, indicating the attestation statement format
    // - authData: a byte string, containing the authenticator data
    // - attStmt: a map, containing the attestation statement
    ...
    err = cbor_parser_init(cbor_data, cbor_data_len, 0, &parser, &it);
    if (err) {
        debug_printf(DEBUG_LEVEL_ERROR, "Failed to initialize CBOR parser: %d",
                     err);
        return -1;
    }
    if (!cbor_value_is_map(&it)) {
    ...
    // Parse the fmt field
    err = cbor_value_map_find_value(&it, "fmt", &value);
    if (err) {
        debug_printf(DEBUG_LEVEL_ERROR, "Could not find 'fmt' in the map: %d",
                     err);
        return -1;
    }
    if (!cbor_value_is_text_string(&value)) {
        debug_printf(DEBUG_LEVEL_ERROR, "fmt value is not a text string: %d",
                     err);
        return -1;
    }
    ...
    if (err != CborNoError) {
        debug_printf(DEBUG_LEVEL_ERROR,
                     "Could not calculate length of fmt value");
        return -1;
    }
    char fmt[len + 1];
    fmt[len] = '\0';
    err = cbor_value_copy_text_string(&value, fmt, &len, NULL);
    if (err != CborNoError) {
        debug_printf(DEBUG_LEVEL_ERROR, "Could not copy fmt value");
        return -1;
    }
    // Parse the authData field
    err = cbor_value_map_find_value(&it, "authData", &value);
    if (err) {
        debug_printf(DEBUG_LEVEL_ERROR,
                     "Could not find 'authData' in the map: %d", err);
        return -1;
    }
    err = cbor_value_calculate_string_length(&value, &len);
    if (err != CborNoError) {
        debug_printf(DEBUG_LEVEL_ERROR,
                     "Could not calculate length of authData value");
        return -1;
    }
    data->authdata_len = len;
    data->authdata = OPENSSL_malloc(len);
    err = cbor_value_copy_byte_string(&value, data->authdata, &len, NULL);
    if (err != CborNoError) {
        debug_printf(DEBUG_LEVEL_ERROR, "Could not copy authData value");
        return -1;
    }
    debug_print_hex(DEBUG_LEVEL_MORE_VERBOSE, "authData: ", data->authdata,
                    data->authdata_len);

    // If fmt = "none", the authenticator indicates self attestation. This means
    // the attStmt is empty, and the authenticator data is the only source of
    // attestation. Currently, this is the only supported format.
    // TODO: support other formats
    assert(strcmp(fmt, "none") == 0);
...
//
//     u8 *buf = malloc(1500);
//     CborEncoder array, encoder, map;
//     cbor_encoder_init(&encoder, buf, 1500, 0);
//     CborError err;
//
//     cbor_encoder_create_array(&encoder, &array, 2);
//
//     uint8_t binary[] = { 0x68, 0x65, 0x6C, 0x6C, 0x6F };
//
//     printf("Encoding integer\n");
//     err = cbor_encode_int(&array, 1);
//     if (err != CborNoError) {
//         printf("Error encoding integer\n");
//         return -1;
//     }
//     printf("Encoding byte string\n");
//     err = cbor_encode_byte_string(&array, binary, 5);
//     if (err != CborNoError) {
//         printf("Error encoding byte string\n");
//         return -1;
//     }
//     printf("Getting buffer size\n");
//     size_t len = cbor_encoder_get_buffer_size(&array, buf);
//
//     debug_print_hex(DEBUG_LEVEL_VERBOSE, "Encoded data: ", buf, len);
//
//     cbor_encoder_close_container(&encoder, &array);
//
//     cbor_encoder_create_map(&encoder, &map, 1);
//     cbor_encode_text_stringz(&mapEncoder, "a");
// cbor_encode_int(&mapEncoder, 4);
// cbor_encode_text_stringz(&mapEncoder, "b");
// cbor_encode_int(&mapEncoder, 5);
...
//     printf("Initializing CBOR parser\n");
//     err = cbor_parser_init(buf, len, 0, &parser, &it);
//     if (err != CborNoError) {
//         printf("Error initializing CBOR parser\n");
//         return -1;
//     }
//     int value;
//     if(!cbor_value_is_array(&it)) {
//         printf("Expected array at the beginning of the data\n");
//         return -1;
//     }
//     cbor_value_enter_container(&it, &it);
//     if (!cbor_value_is_integer(&it)) {
//         printf("Expected integer at the beginning of the data\n");
//         return -1;
//     }
//     printf("Decoding integer\n");
//     err = cbor_value_get_int(&it, &value);
//     if (err != CborNoError) {
//         printf("Error decoding integer\n");
//         return -1;
//     }
//     printf("Decoded integer: %d\n", value);
//
//     printf("Advancing to the next item\n");
//     err = cbor_value_advance(&it);
//     if (err != CborNoError) {
//         printf("Error advancing to the next item\n");
//         return -1;
//     }
//     // Debugging: Check if at the end or invalid state
//     if (cbor_value_at_end(&it)) {
//         fprintf(stderr, "Iterator at end of data\n");
//         return -1;
//     } else {
//         CborType nextType = cbor_value_get_type(&it);
//         fprintf(stderr, "Current type after advancing: %d\n", nextType);
//     }
//
//     uint8_t out[5];
...
//         fprintf(stderr, "Expected byte string but found type: %d\n", type);
//         return -1;
//     }
//     if (!cbor_value_is_byte_string(&it)) {
//         printf("Expected byte string\n");
//         return -1;
//     }
//     printf("Decoding byte string\n");
//     cbor_value_copy_byte_string(&it, out, &out_len, &it);
//
//     debug_print_hex(DEBUG_LEVEL_VERBOSE, "Decoded byte string: ", out,
//     out_len);
//
//     return 0;
//
